...
class AthleteForm extends Component {
  ...
  submit(e) {
    ...
    bodyFormData.append("sportGroupName", this.state.sportGroupName)
    ...
    axios
      .post(
        `${baseURL}/wp-json/contact-form-7/v1/contact-forms/918/feedback`,
        bodyFormData,
        config
      )
      .then(res => {
  ...
  render() {
    const title = this.props.data.jobPostingTitle
    const content = this.props.data.jobPostingContent
    const active = this.props.data.jobPostingActive

    let errorYourName = false
    let errorYourEmail = false
    let errorSportGroupName = false
    ...
    if (this.state.errors.length > 0) {
      this.state.errors.forEach(error => {
        ...
        } else if (error.idref === "sportGroupName") {
          errorSportGroupName = error.message
        } else if (error.idref === "position") {
          errorPosition = error.message
        } else if (error.idref === "link") {
          errorLink = error.message
        }
      })
    }
        ...
        <StandardWrapper className="athleteform__wrapper">
          ...
          {active === "yes" && (
            <form onSubmit={this.submit} className="athleteform__form">
              ...
              <div className="athleteform__form--field">
                <label htmlFor="sportGroupName">Sport Group Name</label>
                <TextFieldStyled>
                  {errorSportGroupName && (
                    <p className="error-warning">{errorSportGroupName}</p>
                  )}
                  <input
                    type="text"
                    id="sportGroupName"
                    name="sportGroupName"
                    value={this.state.sportGroupName}
                    onChange={this.onChange}
                    required={false}
                  />
                </TextFieldStyled>
              ...
              <div className="athleteform__form--field">
                ...
                <TextFieldStyled>
                  {errorPosition && (
                    <p className="error-warning">{errorPosition}</p>
                  )}
                  <input
                    type="text"
                    id="position"
              ...
              <div className="athleteform__form--field">
                <label htmlFor="link">
                  Your Website link for more information
                </label>
                <TextFieldStyled>
                  {errorLink && <p className="error-warning">{errorLink}</p>}
                  <input
                    type="text"
                    name="link"
                    id="link"
